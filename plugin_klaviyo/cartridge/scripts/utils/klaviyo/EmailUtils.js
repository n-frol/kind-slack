...
/**
 * Sends an order to Klaviyo based on param format from the OrderTransactioanEmails.execute() call
 * @param  toEmail
 * @param  param
 * @param  emailType
 */
function sendOrderEmailCustom(toEmail, param, emailType) {

	var orderPayload = prepareOrderPayloadFromParam(param, emailType);
	var KlaviyoUtils = require('*/cartridge/scripts/utils/klaviyo/KlaviyoUtils');

	// FR added in a boolean return into the sendmail to indicate success or failure
	return KlaviyoUtils.sendEmail(toEmail, orderPayload, emailType);

}

/**
 * Prepares the order in JSON format for email send. Fred Custom for KIND shipment and cancel e-mails
 * @param param
 * @param mailType
 */
function prepareOrderPayloadFromParam(param, mailType) {

	var orderDetails = {};
	var productLineItems = '';
	var orderNo = param.get('OrderNumber');
	...
	if (!empty(order)) {
		productLineItems = JSON.parse(param.get('productLineItemsRaw'));
		for(var j in productLineItems) {
		    ...
		    var priceString = '';
			var priceValue = 0.0;
			// added in support for display of the line item discount FR
			var grossPriceString = '';
			var grossPrice = 0.0;
		    ...
		    if(!productLineItem.bonusProductLineItem) {
				priceString = dw.util.StringUtils.formatMoney(dw.value.Money(productDetail.getPriceModel().price.value, session.getCurrency().getCurrencyCode()));
				grossPriceString = dw.util.StringUtils.formatMoney(dw.value.Money(productDetail.getPriceModel().price.value, session.getCurrency().getCurrencyCode()));
				adjustedPriceString = dw.util.StringUtils.formatMoney(dw.value.Money(productDetail.getPriceModel().price.value, session.getCurrency().getCurrencyCode()));
				grossPrice = productDetail.getPriceModel().price.value
				priceValue = productDetail.getPriceModel().price.value;
				if(productLineItem.price && productLineItem.price.value == 0)
				{
					continue;
				}
		    } else {
			...
			itemCount = itemCount + productLineItem.productQuantityShipped;
			...
			productLineItemsArray.push({
		        ...
		        'QUANTITY': productLineItem.productQuantityShipped,
				'PRICE': priceString,
				'BASE_PRICE': grossPriceString,
				'BASE_PRICE_RAW': grossPrice,
				'DISCOUNT': grossPrice,
				'DISCOUNT_FORMATTED': adjustedPriceString,
		        'PRODUCT_URL': prdUrl,
				'PRODUCT_VARIANT': variationValues,
				'PRICE_VALUE': grossPrice,
		...
		// FRED added in merchandise total
		orderDetails['MERCHANDISE_TOTAL'] = merchandiseTotalString;
	...
	billingaddress.push({
        'FIRST_NAME': param.get('billingFirstName'),
        ...
        'POSTAL_CODE': orderBillingAddressPostalCode,
	...
	shippingaddress.push({
        ...
        'CITY': param.get('city'),
        'POSTAL_CODE': param.get('postalCode'),
        'STATE_CODE': param.get('state'),
	...
	// Klaviyo Customization
	orderDetails['TRACKING_URL'] = param.get('trackingURL');
	orderDetails['TRACKING_NUMBER'] = param.get('trackingNumber');
	orderDetails['AMOUNT_REFUNDED'] = param.get('amountRefunded');

	// Klaviyo Fred added in Channel Type
	orderDetails['CHANNEL_TYPE'] = order.channelType.displayValue;
	...
	if(order.shipments.length > 0){
		...
		for(var j in productLineItems) {
		    ...
		    if(!productLineItem.bonusProductLineItem) {
				...
				grossPriceString = dw.util.StringUtils.formatMoney(dw.value.Money(productLineItem.grossPrice.value, session.getCurrency().getCurrencyCode()));
				adjustedPriceString = dw.util.StringUtils.formatMoney(dw.value.Money(productLineItem.adjustedPrice.value, session.getCurrency().getCurrencyCode()));
				grossPrice = productLineItem.grossPrice.value;
				priceValue = productLineItem.price.value;
				// do not list BYOB components in order confirmation e-mail
				if(productLineItem.price.value == 0)
				{
					continue;
				}
		    } else {
			...
			productLineItemsArray.push({
				...
				'PRODUCT_IMG_URL': productDetail.getImage("large").getAbsURL().toString(),
				'END_USE_CODE': productLineItem.getCustom().endUseCode,
		        'IS_SAMPLE': isSample
		    });
...
module.exports = {
	...
	sendOrderEmailCustom: sendOrderEmailCustom,
	prepareOrderPayloadFromParam: prepareOrderPayloadFromParam
};
