...
function ProfileUpdater($form, LRObject) {
    ...
    this.unverifyAccountUrl = this.$profileForm.data('lrUnverifyAccountUrl');
...
function updateLRProfile() {
    var instance = this;
    var postUrl = this.unverifyAccountUrl;

    var data = {
        email: [{ Type: 'Primary', Value: this.updateData.email }],
        firstname: this.updateData.firstName,
        lastname: this.updateData.lastName
    };

    // Unverify account to allow changing email address
    $.ajax({
        url: postUrl,
        method: 'POST',
        ...
        instance.LRObject.api.updateData(
            instance.schema, data, instance.token,
            ...
            instance.handleLRErrors.bind(instance)
        );
    });
}
