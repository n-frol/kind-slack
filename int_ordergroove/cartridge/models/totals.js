    ...
    collections.forEach(collection, function (item) {
        if (!item.basedOnCoupon) {
            result[item.UUID] = {
                ...
                type: 'promotion',
                callOutMsg: !empty(item.promotion) && !empty(item.promotion.calloutMsg) ? item.promotion.calloutMsg : null
            };
        }
    });
    ...
    collections.forEach(lineItemContainer.couponLineItems, function (couponLineItem) {
        var priceAdjustments = collections.map(
            couponLineItem.priceAdjustments, function (priceAdjustment) {
                return { callOutMsg: !empty(priceAdjustment.promotion) && !empty(priceAdjustment.promotion.calloutMsg) ? priceAdjustment.promotion.calloutMsg : '' };
            });
...
function totals(lineItemContainer) {
    if (lineItemContainer) {
        ...
        this.subTotalDecimal = lineItemContainer.getAdjustedMerchandizeTotalPrice(false).value;
        this.subTotalDecimalIncludingOrderLevelDiscounts = lineItemContainer.getAdjustedMerchandizeTotalPrice(true).value;
        this.totalShippingCost = getTotals(lineItemContainer.shippingTotalPrice);
        this.totalShippingCostDecimal = !lineItemContainer.shippingTotalPrice.available ? '-' : lineItemContainer.shippingTotalPrice.value;
        if (this.totalShippingCost === '-') {
            this.totalTax = '-';
            this.grandTotal = '-';
        } else {
            this.grandTotal = getTotals(lineItemContainer.totalGrossPrice);
            this.grandTotalDecimal = lineItemContainer.totalGrossPrice.value;
            this.totalTax = getTotals(lineItemContainer.totalTax);
            this.totalTaxDecimal = lineItemContainer.totalTax.value;
        }
