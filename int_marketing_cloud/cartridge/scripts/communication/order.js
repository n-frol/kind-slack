 ...
 * @returns {SynchronousPromise}6
 */
function confirmation(promise, data) {
    var xml = helpers.stripXmlNS( data.params.Order.getOrderExportXML(null, null) );
    data.orderAsXML = helpers.getModXML(xml);
    checkWeight(data.params.Order);
    return sendTrigger(hookPath + 'confirmation', promise, data);
}

function checkWeight(order) {
    if (!empty(order)) {
        ...
        pliCollection.toArray().forEach(function (pli) {
            ...
            var weight = productCustom.weight;
            if (weight == "undefined") 
            { 
                return; 
            }
            var weightNumber = isNaN(Number(weight)) ?
                Number(weight) : 0;
            w = w + (weight*pli.getQuantity());
        });
        var a =w;
        if (w >= 150) {
            var qq = order.getShipments();
            var contactEmail = new Mail();
            var array = dw.system.Site.current.getCustomPreferenceValue("b2bweightemail").split(',');
            array.forEach(function(e) {
                contactEmail.addTo(e.trim());
            });
            contactEmail.setSubject("Weight over 150 orderid: " + order.getOrderNo());
            var content = "Order information as follows:";
            content += "Order ID: "+order.getOrderNo();
            content += "Lift Gate: "+(order.getShipments()[0].custom.liftgate ? order.getShipments()[0].custom.liftgate : "null");
            content += "Instructions: "+(order.getShipments()[0].custom.delivery_instruct ? order.getShipments()[0].custom.delivery_instruct: "null");
            content += "Name: "+order.getCustomerName();
            content += "Email: "+order.getCustomerEmail();
            content += "Weight: "+(w ? w: "0");
            contactEmail.setFrom("ksdev@kindsnacks.com");
            var mimeType = 'text/html';
            var encoding = 'UTF-8';
            contactEmail.setContent(content, mimeType, encoding);
            var worked = contactEmail.send();
        }
    }
}
