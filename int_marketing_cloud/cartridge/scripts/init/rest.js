...
ServiceRegistry.configure('marketingcloud.rest.auth.b2c', {
    /**
     * Create request for service authentication
     * @param {dw.svc.HTTPService} svc
     * @throws {Error} Throws error when service credentials are missing
...
ServiceRegistry.configure('marketingcloud.rest.messaging.send.b2c', {
    /**
     * Create request for sending an email
     * @param {dw.svc.HTTPService} svc
     * @param {module:models/message~Message} message A message model instance to be sent to Marketing Cloud
...
ServiceRegistry.configure('marketingcloud.rest.interaction.events', {
    ...
    createRequest: function(svc, event) {
        ...
        event.EventDefinitionKey = Site.getCurrent().getCustomPreferenceValue('mcWelcomeCouponEventDefinitionKey');
        setAuthHeader(svc);
...
ServiceRegistry.configure('marketingcloud.rest.interaction.events.b2c', {
    /**
     * Create request for posting an event
     * @param {dw.svc.HTTPService} svc
     * @param {module:models/event~Event} event An event model instance to be sent to Marketing Cloud
 ...
 * Name: Post Event
 * Documentation: https://developer.salesforce.com/docs/atlas.en-us.noversion.mc-apis.meta/mc-apis/postEvent.htm
 * Endpoint: /data/v1/async/dataextensions/key:{id}/rows
 * Method: POST
 * Content Type: JSON
 * Request Headers: Authorization
 * Request Args: -
 */
ServiceRegistry.configure('marketingcloud.rest.dataextension.insert', {
    /**
     * Create request for posting an event
     * @param {dw.svc.HTTPService} svc
     * @param {module:models/event~Event} event An event model instance to be sent to Marketing Cloud
     * @returns {string} Request body
     */
    createRequest: function(svc, record) {

        var mcWelcomeCouponDataExtensionExternalKey = Site.getCurrent().getCustomPreferenceValue('mcWelcomeCouponDataExtensionExternalKey');

        // Per KIND, they want the subscriber key set to '0'.
        var svcURL = svc.getConfiguration().credential.URL,
            svcPath = '/hub/v1/dataevents/key:' + mcWelcomeCouponDataExtensionExternalKey + '/rows/Email:' + record.values.Email + ',SubscriberKey:0';
...
ServiceRegistry.configure('marketingcloud.rest.dataextension.insert.b2c', {
    /**
     * Create request for posting an event
     * @param {dw.svc.HTTPService} svc
     * @param {module:models/event~Event} event An event model instance to be sent to Marketing Cloud
