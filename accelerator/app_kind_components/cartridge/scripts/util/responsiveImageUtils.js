 ...
 * @param {string} format - Override image format
 * @returns {string} the encoded image URL
 *
 */
function getTransformedImage(image, imageSize, format) {
    var imageTransformations = {
        scaleWidth: imageSize
    };
    if (format) {
        imageTransformations.format = format;
    }

    return urlEncodeSpaces(image.getAbsImageURL(imageTransformations));
}
 ...
 *
 */
function getResponsiveImage(image, imageSizeDefaultIn, imageSizeLargeIn, imageAltExplicit, format) {
    ...
    if (!empty(image)) {
        if (!empty(imageSizeDefault)) {
            imageUrlDefault = getTransformedImage(image, imageSizeDefault, format);
        }

        if (!empty(imageSizeLarge)) {
            imageUrlLarge = getTransformedImage(image, imageSizeLarge, format);
        }
