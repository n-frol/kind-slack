/**
 * Replaces the base version in order to add absolute URLs to the product images
 * model for use in 3rd party api calls & product feeds, like PowerReviews.
 */
    ...
    imageConfig.types.forEach(function (type) {
        ...
        if (imageConfig.quantity === 'single') {
            ...
            if (firstImage) {
                result = [{
                    alt: firstImage.alt,
                    absUrl: firstImage.httpsURL.toString(),
            ...
            result = collections.map(images, function (image) {
                return {
                    alt: image.alt,
                    absUrl: image.httpsURL.toString(),
