/**
 * Imported development note
 * This module added for backward compatibility, new version of the SFRA module
 * is using different action endpoint for ajax request and refinments function is wrapped with
 * cache that's not working with old system.
 *
 * Only way to resolve issue to overwriting the new version with old.
 * New development in this area must consider replace this file with new version.
 */
...
function ProductSearch(productSearch, httpParams, sortingRule, sortingOptions, rootCategory) {
    ...
    if (productSearch.category) {
        this.category.categorySlug = productSearch.category.custom.categorySlug;
        var site = Site.getCurrent();
        var byobCat = site.getCustomPreferenceValue('byobRootCategoryID');
        var isBYOB = !empty(byobCat) && productSearch.category.ID === byobCat;

        // If this is a configuration category, then it is also a BYOB category.
        if (!empty(productSearch.category.custom.isConfigurationCategory) &&
            productSearch.category.custom.isConfigurationCategory === true
        ) {
            isBYOB = true;
        }

        this.isBYOBSearch = isBYOB;
    }
}
