 ...
 * @param {*} email - email used in shipping
 */
function updateAddressFields(newAddress, address, email) {
 ...
 * @param {string} email - email to copy in
 * @returns {void}
 */
function saveAddress(address, customer, addressId, email) {
    var Transaction = require('dw/system/Transaction');

    var addressBook = customer.raw.getProfile().getAddressBook();
    Transaction.wrap(function () {
        var newAddress = addressBook.createAddress(addressId);
        updateAddressFields(newAddress, address, email);
    });
}
