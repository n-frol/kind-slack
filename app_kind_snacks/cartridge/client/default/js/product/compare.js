...
var $compareBar = $('.js-compare-bar');
var $compareBarInner = $compareBar.find('.js-compare-bar-inner');
var maxSlots = parseInt($compareBarInner.data('max-slots'), 10);
...
function compileSlot(product, idx) {
    ...
    return '' +
        '<div class="col-3 selected-product">' +
            '<div class="c-compare-bar__slot js-slot" data-pid="' + pid + '">' +
                '<img class="c-compare-bar__slot__img" src="' + product.imgSrc + '" />' +
                '<button class="c-compare-bar__slot__close pull-right js-close" aria-label="remove product from compare">' +
                    '<div class="c-compare-bar__slot__close__icon"></div>' +
                '</button>' +
            '</div>' +
    ...
    if (productsToCompare.length < maxSlots) {
        ...
        for (var i = 0; i < numAvailableSlots; i++) {
            if (i === 0 && productsToCompare.length < 2) {
                html += '<div class="col-3 selected-product">' +
                    '<div class="c-compare-bar__slot js-slot">' +
                    '<div class="min-products-msg c-compare-bar__slot__min-products-msg">' +
                    $compareBarInner.data('min-products-msg') +
                    '</div></div></div>';
            } else {
                html += '<div class="col-3 selected-product">' +
                    '<div class="c-compare-bar__slot js-slot"></div></div>';
            }
        }
    }

    $compareBarInner.find('.js-product-slots').empty().append(html);
}
...
module.exports = {
    ...
    handleClearAll: function () {
        $compareBarInner.find('.clear-all').on('click', function (e) {
            e.preventDefault();
            clearCompareBar();
        });
    },
    ...
    deselectProductOnCompareBar: function () {
        $compareBarInner.on('click', '.js-close', function () {
            var pid = $(this).closest('.js-slot').data('pid').toString();
            productsForComparison = deselectProduct(productsForComparison, pid);
            $(this).trigger('compare:deselected', { pid: pid });
        });
    },
     ...
     */
    setBackUrl: function () {
        var $backUrl = $('input[name="backUrl"]');
        var currentLocation = window.location;

        var backUrl = currentLocation.href.replace(currentLocation.hash, '');
        // Remove previously set comparePids so they don't end up stacking
        if (currentLocation.search) {
            var searchArr = currentLocation.search.split('&');
            var len = searchArr.length;

            for (var i = 0; i < len; i++) {
                var term = searchArr[i];

                if (term.indexOf('comparePids') !== -1) {
                    backUrl = backUrl.replace((i === 0 ? '' : '&') + term, '');

                    // Don't break immediately in case duplicates snuck their way in
                }
            }
        }

        $backUrl.val(backUrl);
        $('.container').on('click', '.refinements a', function () {
            $backUrl.val($(this).prop('href'));
        });
    },
