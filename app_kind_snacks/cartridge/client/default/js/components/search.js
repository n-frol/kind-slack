 ...
 * search.js
 *
 * Overrides some of the defult SFRA search functionality.
 */
...
function getSuggestionsWrapper(scope) {
    var $wrapper = $(scope).siblings('.suggestions-wrapper');
    var $byobWrapper = $('.js-byob-search-wrapper');

    if (!$wrapper.length && $byobWrapper.length) {
        $wrapper = $byobWrapper;
    }
...
/**
 * Gets the endpoint to submit search requests to from the data attribute on
 * the search suggestions container element.
 *
 * @param {JQuery} scope - The search form jQuery element.
 * @returns {string} - Returns the URL string.
 */
function getEndpoint(scope) {
    return getSuggestionsWrapper(scope).data('url');
}
...
function getSuggestions(scope) {
    if ($(scope).val().length >= minChars) {
        ...
        $.ajax({
            context: scope,
            url: getEndpoint(scope) + encodeURIComponent($(scope).val()),
            method: 'GET',
...
module.exports = {
    ...
    /**
     * Export all the functions as members of the export object so they can
     * be accessed and/or overridden from outside of the module.
     */
    methods: {
